module BohconfSessionsHelper
	def session_time(time)
		...
		if Time.zone.now < Time.zone.parse("2012-04-23 10:00pm")
			...
			(1..10).each do |i|
				options << ["Monday #{i}:00pm", Time.zone.parse("2012-04-23 #{'%02d' % (i+12)}:00pm")]
				options << ["Monday #{i}:30pm", Time.zone.parse("2012-04-23 #{'%02d' % (i+12)}:30pm")]
			end
		end
		...
		if Time.zone.now < Time.zone.parse("2012-04-24 9:00pm")
			...
			(1..9).each do |i|
				options << ["Tuesday #{i}:00pm", Time.zone.parse("2012-04-24 #{'%02d' % (i+12)}:00pm")]
				options << ["Tuesday #{i}:30pm", Time.zone.parse("2012-04-24 #{'%02d' % (i+12)}:30pm")]
			end
		end
		...
		(1..3).each do |i|
			options << ["Wednesday #{i}:00pm", Time.zone.parse("2012-04-25 #{'%02d' % (i+12)}:00pm")]
